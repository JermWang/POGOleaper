...
export async function POST(req: Request) {
  ...
  try {
    ...
    //  - PUBLIC tables may ONLY be denominated in the house token.
    //  - PRIVATE tables may use ETH, USDC, or the token.
    //  - The token is only selectable once its contract is configured.
    ...
    if (c.visibility === "PUBLIC" && c.asset !== "TOKEN") {
      return NextResponse.json(
        {
          error: `Public tables must use ${env.tokenSymbol}. Choose ETH or USDC only for private tables.`,
        },
        { status: 400 },
      );
    }
