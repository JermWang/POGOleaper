 ...
 * ones, and NO user identity (just amount, asset, time, and the public
 * Blockscout link). Edge-cached so a high-traffic splash doesn't hammer the DB.
 */
export async function GET() {
  try {
    const rows = await prisma.withdrawal.findMany({
      ...
      take: 30,
      select: { asset: true, amount: true, txHash: true, updatedAt: true },
    });
    const payouts = rows.map((r) => ({
      amount: formatAmount(r.asset, r.amount),
      sym: ASSET_SYMBOLS[r.asset],
      at: r.updatedAt.toISOString(),
      url: r.txHash ? explorerTxUrl(r.txHash) : null,
    }));
