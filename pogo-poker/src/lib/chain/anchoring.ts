 ...
 * only the ROOT is posted on-chain — in the calldata of a single 0-value tx from
 * the hot wallet to itself — so outcomes/payouts are tamper-evident in one cheap
 * transaction, without publishing per-hand data or player identities on-chain.
 *
 ...
 * one in the on-chain anchor tx's calldata (decode it as utf-8 on Blockscout).
 * The verify endpoint returns exactly that bundle.
 *
...
// "-evm-v1": same Merkle/leaf construction as the Solana-era "-v1", but the
// root is now committed in EVM tx calldata instead of a Solana memo. Hands
// anchored going forward carry this id; the hashing itself is unchanged.
export const ANCHOR_ALGORITHM = "velvet-anchor-merkle-sha256-evm-v1";
export const ANCHOR_PAYLOAD_PREFIX = "velvet-anchor-evm-v1";
...
/** The compact string posted on-chain in the anchor tx's calldata (utf-8). */
export function buildAnchorPayload(rootHex: string, handCount: number): string {
  return `${ANCHOR_PAYLOAD_PREFIX}:${rootHex}:${handCount}`;
}
