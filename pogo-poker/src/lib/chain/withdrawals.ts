 ...
 *  - Sending uses the chain provider; the tx hash is persisted.
 ...
 * on amount threshold (ETH wei basis; USDC uses an equivalent threshold).
 */
...
export async function sendApprovedWithdrawal(
  ...
  if (claim.count !== 1) {
    return { txHash: "" }; // not ours to send (already claimed/sent/not approved)
  }
...
function velocityAmountCap(asset: Asset): bigint {
  switch (asset) {
    case "ETH":
      return env.withdrawalDailyMaxWei;
    case "USDC":
...
function aboveReviewThreshold(asset: Asset, amount: bigint): boolean {
  switch (asset) {
    case "ETH":
      return amount >= env.minWithdrawalReviewWei;
    case "USDC":
