 ...
 * chain provider falls back to the mock and the loops simply find nothing to do.
 */
...
export function startBackgroundWorkers(): void {
  ...
  console.log(
    `[worker] starting (chainId=${env.robinhoodChainId}, on-chain=${onChain ? "live" : "mock"})`,
  );
  ...
  // mock mode they would write fake tx hashes / fake anchors into the live DB,
  ...
  if (!onChain) {
    console.warn(
      "[worker] on-chain workers idle — set HOT_WALLET_PRIVATE_KEY (+ TREASURY_WALLET_ADDRESS) to enable deposits, withdrawals, and outcome anchoring on Robinhood Chain.",
    );
    return;
  }
