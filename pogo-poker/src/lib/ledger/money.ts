...
export type Asset = "ETH" | "USDC" | "TOKEN";
...
const TOKEN_DECIMALS =
  Number(process.env.NEXT_PUBLIC_TOKEN_DECIMALS ?? "18") || 18;
...
/** Display ticker for an asset (ETH/USDC literal; TOKEN uses the configured symbol). */
...
export function parseEthToWei(eth: string): bigint {
  return parseDecimalToBaseUnits(eth, ASSET_DECIMALS.ETH);
}

export function formatWeiToEth(wei: bigint): string {
  return formatBaseUnitsToDecimal(wei, ASSET_DECIMALS.ETH);
}
...
/** Human-friendly label, e.g. "1.5 ETH", "25 USDC", or "100 POGO". */
