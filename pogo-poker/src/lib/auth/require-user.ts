...
export async function getCurrentUser(): Promise<User | null> {
  ...
  if (!existing) {
    ...
    const refCode = cookies().get("pogo_ref")?.value;
  ...
  // Persist the linked wallet(s) for cashier/withdrawal UX and audit.
  ...
  // EVM addresses are stored LOWERCASE (checksum casing varies by source) so
  // deposit-sender matching is case-insensitive by construction.
  for (const rawAddress of identity.wallets) {
    const address = rawAddress.toLowerCase();
    const existing = await prisma.wallet.findUnique({
      where: { chain_address: { chain: "ROBINHOOD", address } },
    });
    if (!existing) {
      await prisma.wallet.create({
        data: { userId: user.id, chain: "ROBINHOOD", address, type: "EMBEDDED" },
      });
