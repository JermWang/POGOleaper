...
/** The branded pogo poker chip (black + gold "V"), from /public. */
...
export function PokerTableView(props: PokerTableViewProps) {
  ...
  const [guestId] = useState<string | null>(() => {
    ...
    try {
      const existing = window.sessionStorage.getItem("pogo-guest-id");
      if (existing) return existing;
      const id = fresh();
      window.sessionStorage.setItem("pogo-guest-id", id);
      return id;
    } catch {
      return fresh();
    }
  ...
  async function buyIn(amount: string, password?: string, seatNumber?: number) {
    ...
    try {
      baseUnits = parseAmount(props.asset, amount);
    } catch {
      return; /* the field guides format */
    }
    // Frictionless top-up: if the in-app playable balance is short, pull the
    // shortfall straight from the connected wallet (wallet popup), wait for it
    // to credit, then sit — no Cashier round-trip. ETH real-money tables only
    ...
    if (!props.demo && props.asset === "ETH" && canDepositFromWallet()) {
      const available = await fetchPlayableAvailable(props.asset);
      const shortfall = baseUnits - available;
      if (shortfall > 0n) {
        await depositEthFromWallet(shortfall);
        await waitForCredit(props.asset, baseUnits);
      }
    }
    send({
      t: "BUY_IN",
    ...
    <div className="flex h-[calc(100dvh-6.5rem)] flex-col gap-2.5">
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {props.demo && (
            <span
              className="hidden rounded-full border border-pogo/25 bg-pogo/[0.06] px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-pogo/90 sm:inline-block"
          ...
          {isSpectator && (
            <span className="rounded-full border border-pogo/30 bg-pogo/5 px-2.5 py-1 text-[10px] uppercase tracking-[0.16em] text-pogo/90">
              Spectating
            </span>
          )}
          <span
          ...
          {voiceAvailable && media.status !== "on" && (
            <Button
              size="sm"
              variant="secondary"
              className="whitespace-nowrap gap-1.5 border border-pogo/45 px-3 text-xs font-semibold"
              onClick={() => media.join()}
      ...
      >
        ...
        <div className="relative h-full w-full sm:aspect-[820/620] sm:mx-auto sm:h-auto sm:max-h-full sm:max-w-3xl">
          ...
          <div className="pointer-events-none absolute" style={{ inset: "3.2% 8.1%" }}>
            <div
              style={{
                ...
                boxShadow:
                  "0 44px 90px -38px rgba(0,0,0,0.85), inset 0 2px 0 rgba(255,255,255,0.06), inset 0 0 0 1px rgba(74,222,128,0.26), inset 0 -8px 20px rgba(0,0,0,0.5)",
              }}
            >
              <div
                style={{
          ...
          <div className="absolute inset-x-[10%] inset-y-[24%] flex flex-col items-center justify-center gap-2 sm:inset-x-[16%] sm:inset-y-[30%] sm:gap-3">
            ...
            {table && (
              ...
              <div
                key={table.totalPot}
                className="animate-pot-pop flex items-center gap-2 rounded-full border border-pogo/30 bg-charcoal-900/55 px-3.5 py-1.5 backdrop-blur"
              >
                <Chip size={22} />
                <span className="text-[11px] text-ash">Pot</span>
                <span className="font-mono text-sm text-pogo-soft">
              ...
              >
                ...
                {!s.playerId && props.demo && !seated && !isSpectator ? (
                  <button
                    ...
                    className="grid h-11 w-11 place-items-center rounded-full border border-dashed border-pogo/60 bg-pogo/10 text-[9px] font-semibold uppercase tracking-wide text-pogo transition-colors hover:border-pogo hover:bg-pogo/25 hover:text-ivory"
                  >
                    Sit
                  </button>
                ) : (
        ...
        {isSpectator ? (
          ...
          props.demo ? (
            <div className="rounded-2xl border border-pogo/40 bg-charcoal-800/90 p-4 text-center shadow-elevated">
          ...
          <div className="rounded-2xl border border-pogo/30 bg-pogo/[0.06] p-3 text-center">
          ...
          <div className="rounded-2xl border border-white/10 bg-charcoal-800/70 p-3.5 shadow-elevated">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-pogo-soft">
                  ...
                  {(
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        preAction === key
                          ? "border-pogo bg-pogo/25 text-ivory"
  ...
  useEffect(() => {
    ...
    if (isYourTurn && !prev.current) {
      ...
      if (typeof document !== "undefined") {
        document.title = "🔔 Your turn — Pogo Poker";
      }
