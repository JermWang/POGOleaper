...
export function ActionBar({
  ...
  return (
    <div className="animate-attn space-y-3 rounded-2xl border border-pogo/40 bg-charcoal-800/90 p-3.5 shadow-elevated">
      <div className="flex items-center justify-between px-1">
        <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-pogo-soft">
          Your turn
        </span>
        {secondsLeft != null && (
          <span
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {presets.map((p) => (
              <button
                ...
                className="rounded-lg border border-pogo/30 bg-pogo/[0.08] px-2.5 py-1 text-xs font-medium text-pogo-soft transition-colors hover:bg-pogo/20"
              >
                {p.label}
              </button>
            ))}
            ...
            <input
              ...
              className="ml-auto w-24 rounded-lg border border-white/12 bg-charcoal-900/60 px-2 py-1 text-right font-mono text-sm text-ivory focus:border-pogo/50 focus:outline-none"
            />
          </div>
            ...
            <div className="absolute bottom-full left-0 z-20 mb-2 w-full max-w-xs rounded-xl border border-white/12 bg-charcoal-900/97 p-3 shadow-elevated backdrop-blur">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-pogo-soft hover:text-ivory"
                >
                  Reset
                </button>
              </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {canRaise && (
          <button
            type="button"
            onClick={submitRaise}
            className="flex-1 rounded-xl bg-pogo px-5 py-3 text-sm font-semibold text-ivory shadow-pogo transition-colors hover:bg-pogo-soft"
            style={{
              boxShadow:
                "0 8px 20px -8px rgba(34,197,94,0.6), inset 0 1px 0 rgba(255,255,255,0.16)",
            }}
          >
        ...
        <button
          ...
          className="rounded-xl border border-pogo/50 bg-pogo/[0.14] px-4 py-3 text-sm font-semibold text-pogo-soft transition-colors hover:bg-pogo/25"
        >
