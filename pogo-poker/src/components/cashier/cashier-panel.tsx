...
export function CashierPanel({
  ...
  tokenSymbol: string;
  /** The player's connected wallet — the one-click withdraw destination. */
  ...
  const [wAsset, setWAsset] = useState("ETH");
  ...
  async function withdraw(e: React.FormEvent<HTMLFormElement>) {
    ...
    setWMessage(
      json.requiresReview
        ...
        : "Withdrawal requested — sending on-chain now. Track its status in your history below; a Blockscout link appears once it lands (and it's auto-refunded if a send ever fails).",
    );
    router.refresh();
  }
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-1.5">
            Deposit
            <HelpHint label="Send funds from the Robinhood Chain wallet you signed in with. We match the deposit to your account automatically and credit it after on-chain confirmations." />
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-ash">
            Send ETH, USDC{tokenConfigured ? `, or ${tokenSymbol}` : ""} (ERC-20)
          ...
          {address ? (
            <div className="rounded-xl border border-white/10 bg-charcoal-900/60 p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs text-ash">Pogo deposit address</p>
                <button
                  type="button"
                  onClick={copyAddress}
          ...
          <p className="text-xs text-ash/70">
            Deposit only from the wallet you signed in with — funds sent
            from another wallet can&apos;t be matched to your account
            automatically. Only send assets on Robinhood Chain; other networks
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-1.5">
            Withdraw
            <HelpHint label="Cash out to any Robinhood Chain address. Small amounts are sent automatically; larger ones get a quick manual review before sending, for safety." />
          </CardTitle>
        </CardHeader>
        <CardContent>
          {!canPlay && (
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-ash/80">
                  ...
                  <button
                    ...
                    className="ml-2 text-[10px] font-semibold uppercase tracking-wide text-pogo transition-colors hover:text-ivory hover:underline underline-offset-2"
                  >
                    Max
                  </button>
                </span>
              ...
              <div className="flex items-center justify-between">
                ...
                {connectedWallet && (
                  <button
                    ...
                    className="text-[10px] font-semibold uppercase tracking-wide text-pogo transition-colors hover:text-ivory hover:underline underline-offset-2"
