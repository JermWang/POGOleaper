 ...
 *
 * A slow domain-warped FBM field rendered as a faint pogo-green ASCII tracery that
 * lives only in the OUTSKIRTS (a radial mask fades it out toward the center so
 * content stays clean), with a soft bloom and a few tiny pogo-green motes. Quiet by
    ...
    const drawFrame = (t: number) => {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          gctx.fillStyle = `rgba(74,222,128,${a.toFixed(3)})`;
      ...
      for (const p of particles) {
        ...
        grd.addColorStop(0.5, `rgba(74,222,128,${(a * 0.32).toFixed(3)})`);
