...
// The USD value as actually shown (rounded), so the ETH line is its exact translation.
function shownUsd(n: number): number {
  return n >= 1 ? Math.round(n) : Math.round(n * 100) / 100;
}
// ETH translation of the headline USD — tidy, ~2 significant figures, no noise.
...
/** Build the USD-headline / ETH-translation pair for a single value or a range. */
function priced(
  ...
  if (parts.every((p) => p.usd != null)) {
    ...
    // amount you'll actually wager. ETH/USDC tables show the ETH equivalent.
    const subtext =
      asset === "TOKEN"
        ? `≈ ${parts.map((p) => p.native).join(sep)} ${sym}`
        : ethUsd
          ? `≈ ${parts.map((p) => fmtEth(shownUsd(p.usd as number) / ethUsd)).join(sep)} ETH`
          : null;
  ...
  // Feed down for ETH/USDC → fall back to the native amount.
...
export function TableCard({
  ...
  const blinds = priced([sb, bb], table.asset, " / ", prices.ethUsd);
  const buyIn = priced([lo, hi], table.asset, "–", prices.ethUsd);
      ...
      <div className="glass glass-hover relative flex h-full flex-col overflow-hidden p-5">
        ...
        <div className="relative flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-col items-end gap-1.5">
            <span className="rounded-full border border-pogo/30 bg-pogo/10 px-2.5 py-0.5 text-xs font-medium text-pogo">
              {currencyTag}
            </span>
        ...
        <div className="relative mt-5 flex items-center justify-between">
          {table.isDemo ? (
            <span className="text-xs text-pogo/80">Free play · no deposit</span>
          ) : table.visibility === "PRIVATE" ? (
