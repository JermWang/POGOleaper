...
export function PayoutTicker() {
  ...
  return (
    <div className="poker-payout-ticker overflow-hidden">
      <style>{`@keyframes pogo-ticker{from{transform:translateX(0)}to{transform:translateX(-50%)}}`}</style>
      <div className="flex h-9 items-center">
        ...
        <div className="relative flex-1 overflow-hidden">
          <div
            ...
            style={{ animation: `pogo-ticker ${duration}s linear infinite` }}
          >
            {track.map((p, i) => {
              const inner = (
                <span className="mx-4 inline-flex items-center gap-1.5 text-xs">
                  ...
                  <span className="font-semibold text-pogo">{p.sym}</span>
