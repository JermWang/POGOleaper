 ...
 * labels it (falls back to the POGO brand ticker). NEXT_PUBLIC_* is inlined at
...
export function ContractAddressChip({
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "border-pogo/30 bg-pogo/[0.08]",
          hasCA
            ? "hover:border-pogo/55 hover:bg-pogo/[0.16]"
            : "cursor-default opacity-70",
          className,
        )}
      >
        <span className="font-semibold tracking-wide text-pogo">CA</span>
        {hasCA ? (
          <>
  ...
  return (
    <button
      ...
      className={cn(
        "glass glass-pogo group flex w-full items-center justify-between gap-3 rounded-2xl px-4 py-3 text-left transition-colors sm:px-5",
        hasCA ? "glass-hover hover:border-pogo/55" : "cursor-default",
        className,
      )}
    >
      <div className="min-w-0">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-pogo sm:text-[11px]">
      ...
      <span
        className={cn(
          ...
          hasCA
            ? "border-pogo/40 bg-pogo/15 text-ivory group-hover:bg-pogo/25"
